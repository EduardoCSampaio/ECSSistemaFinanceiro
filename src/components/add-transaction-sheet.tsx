...
export function AddTransactionSheet({ 
  ...
  const incomeCategories = useMemo(() => 
    categories.filter(c => ['Salário', 'Outras Receitas', 'Transferência'].includes(c.name)), 
  [categories]);
  ...
  useEffect(() => {
    if (isEditing) return;
    transactionForm.setValue('type', activeTab as 'income' | 'expense');
    transactionForm.setValue('categoryId', '');
  }, [activeTab, isEditing, transactionForm])
      ...
      <SheetContent className="sm:max-w-lg">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto py-4 pr-2">
              <Form {...transactionForm}>
                <form id="transactionForm" onSubmit={transactionForm.handleSubmit(onTransactionSubmit)} className="space-y-4">
                  <TabsContent value="expense" forceMount={true} className={cn("space-y-4", activeTab !== 'expense' && 'hidden')}>
                     <FormField
                  ...
                  <TabsContent value="income" forceMount={true} className={cn("space-y-4", activeTab !== 'income' && 'hidden')}>
                     <FormField
                        control={transactionForm.control}
                        name="description"
                        render={({ field }) => (
              ...
              <TabsContent value="transfer" forceMount={true} className={cn(activeTab !== 'transfer' && 'hidden')}>
                 <Form {...transferForm}>
            ...
            <SheetFooter>
                ...
                {activeTab !== 'transfer' && <Button type="submit" form="transactionForm">Salvar {isEditing ? 'Alterações' : (activeTab === 'income' ? 'Receita' : 'Despesa')}</Button>}
